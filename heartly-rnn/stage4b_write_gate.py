#!/usr/bin/env python3
"""
stage4b_write_gate.py — Stage 4b Part A: write-gate formats (PREREG_STAGE4B.md).

Six frozen write formats (W1–W6) teach the same 5 fabricated facts into the
RWKV7 state via prefill; the same 5-question quiz runs after each format
(fresh prefill per question, exactly the stage4_check3 protocol).

Runs on the vast.ai 3090 stack (transformers 4.56.2 + fla 0.5.1, bf16).
Output: stage4b_results/write_gate_report.json + stdout log.
"""
import json
import os
...
def grammar_answer(text):
    return (f"<think>The user asks a personal fact.</think>"
...
def w3_assistant_voice():
    ...
    for f in FACTS:
        parts.append(
            f"User: {f['teach']}\nAssistant: "
            f"<think>The user shared a fact. I now know this.</think>"
...
def w4_trivia():
    ...
    for f in FACTS:
        parts.append(f"User: {f['third_q']}\nAssistant: "
                     f"<think>This is a factual question. I have this in my "
